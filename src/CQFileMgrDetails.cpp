...
selectionChanged()
{
  ...
  for (int i = 0; i < items.size(); ++i) {
    CQFileMgrDetailsRow *item =
      reinterpret_cast<CQFileMgrDetailsRow *>(items.at(i));
...
{
  CQFileMgrDetailsRow *item =
    reinterpret_cast<CQFileMgrDetailsRow *>(currentItem());
...
setColumns(const CFileMgrDetails::ColumnList &columns)
{
  ...
  for (uint i = 0; i < num_columns; ++i) {
    int align = columns[i].align == CHALIGN_TYPE_LEFT ?
