...
CFileMgr::
CFileMgr() :
 type_(VIEW_ICONS), icons_(0), filmstrip_(0), details_(0),
 dir_(0), config_(0), border_(4), show_dot_dot_(true), large_icons_(true),
 show_images_(false), show_hidden_(false), force_rgb_(false)
{
...
setIconSize(CPixelRenderer *renderer)
{
  ...
  icon_size_.width  = int(icon_size*1.25) + 4*border_;
  icon_size_.height = image_size_.height + 5*border_ + char_height;
}
